...
--
--  APPLY TWO PENDING MIGRATIONS IN ORDER: V125 then V126.
...
--  before you start. V125 was staged earlier but not yet applied; V126 must run
--  after it. BOTH are PROC-ONLY re-derivations of the byte-locked
--  SP_LOAD_MARTS_V27 -- V125 re-derives from V113 (one MFA line), and V126
--  re-derives from V125 (the task-graph credit arm). They change NO schema,
--  provision/alter NO compute, and are idempotent (safe to re-run).
--
--  V126 fixes a mart-vs-live split: MART_TASK_GRAPH_DAILY.WH_CREDITS collapsed
--  task auto-retries to the terminal attempt BEFORE joining attribution, so a
--  failed retry's compute dropped from the credit total; the live fallback
--  (graph_sql.graph_daily_costs) sums credits over ALL attempts. So the same
--  Cost > Unit costs > Task-graph "Pipeline spend" flipped with mart warmth.
--  V126 restructures the mart arm to mirror the live twin exactly (keep every
--  attempt, count scheduled tasks via TERMINAL_RN = 1, SUM credits over all
--  attempts), so the two paths agree.
--
--  After both CREATE OR REPLACEs, this file re-runs the DAILY marts ONCE
--  (SP_LOAD_MARTS_V27('DAILY', 3), matching the nightly task) so both the
--  security-posture and task-graph rows re-stamp immediately under the corrected
--  proc, then reconciles each mart against a live recompute.
--
...
--  SCHEMA_VERSION_AFTER = 126, and that BOTH reconciliation RESULTs read
...
-- (byte-identical to snowflake/migrations/ on main)
-- =====================================================================
...
-- (byte-identical to snowflake/migrations/ on main; requires V125 applied first)
-- =====================================================================
...
-- =====================================================================
-- POST-APPLY RE-STAMP: re-run the DAILY marts so BOTH the security-posture row
-- (MFA_GAP_USERS, V125) and the task-graph rows (WH_CREDITS, V126) re-stamp TODAY
-- under the corrected proc. Same call the nightly TASK makes. Idempotent.
-- =====================================================================
...
-- ---- POST-CHECK: expect SCHEMA_VERSION_AFTER = 126 ------------------
-- RESULT:
