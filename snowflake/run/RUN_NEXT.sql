-- =====================================================================
--  OVERWATCH -- RUN_NEXT.sql   (APPLY V139: fix the object-cost loader)
--
--  Confirmed 2026-09-14: FACT_OBJECT_COST_DAILY has been stale since 2026-09-09 because
--  Snowflake renamed SNOWFLAKE.ACCOUNT_USAGE.SEARCH_OPTIMIZATION_HISTORY.TABLE_NAME ->
--  BASE_TABLE_NAME. SP_LOAD_OBJECT_COST's search-opt arm threw 'invalid identifier TABLE_NAME'
--  every night; the atomic load rolled back and kept the 2026-09-09 fill. (DESCRIBE proved
--  clustering/MV still have TABLE_NAME, serverless has TASK_NAME, pipe has PIPE_NAME -- only
--  the search-opt arm was affected.)
--
--  This is migration V139 (also in the repo at snowflake/migrations/V139__object_cost_search_opt_column.sql,
--  shipped in v4.537.0, CI green). It re-derives SP_LOAD_OBJECT_COST from V067 changing ONLY the
--  search-opt arm's object-name column to BASE_TABLE_NAME, then CALLs SP_LOAD_OBJECT_COST(14) to
--  backfill the 2026-09-10..today gap on apply.
--
--  Run All as SNOW_ACCOUNTADMINS (the task/proc owner). Apply after V138 (already applied). The
--  ordering guard aborts safely if V138 is somehow missing. Idempotent: re-running is safe
--  (SCHEMA_VERSION insert is WHERE NOT EXISTS; the proc is CREATE OR REPLACE).
--  ===> After it finishes, run the STEP 2 verification block at the bottom and paste it back. <===
-- =====================================================================

USE ROLE SNOW_ACCOUNTADMINS;
USE WAREHOUSE WH_ALFA_ADMIN;      -- the backfill CALL does real work (ACCESS_HISTORY x QAH, 14d)
...
-- Heal the 2026-09-10..today gap now that the proc compiles (14d, matches the V048 first-fill).
-- Watch the returned value: it MUST be 'OK'. If it says 'FAILED', do NOT paste V139 as done --
-- run STEP 2 [B] to read the new error and send it back.
...
-- [A] Version recorded (expect one row: 139 + the description).
SELECT VERSION, DESCRIPTION FROM DBA_MAINT_DB.OVERWATCH.SCHEMA_VERSION WHERE VERSION = 139;

-- [B] Fresh again? LAST_LOAD_TS should now be ~today (was frozen at 2026-09-09 06:45).
...
-- [C] No NEW failure logged since the apply (expect 0).
SELECT COUNT(*) AS NEW_OBJECT_COST_FAILURES
FROM DBA_MAINT_DB.OVERWATCH.APP_ERROR_LOG
WHERE PAGE = 'ObjectCost'
  AND LOGGED_AT > DATEADD('minute', -15, CURRENT_TIMESTAMP());

-- [D] The search-opt arm now lands rows (was the broken one). Recent per-arm rollup.
SELECT COST_ARM, COUNT(*) AS ROWS_, ROUND(SUM(CREDITS), 2) AS CREDITS
FROM DBA_MAINT_DB.OVERWATCH.FACT_OBJECT_COST_DAILY
