...
--  APPLY V146 + backfill: repoint the FACT_AI_USAGE_DAILY loader's AI-Functions
--  arm onto the canonical CORTEX_AI_FUNCTIONS_USAGE_HISTORY (off the FROZEN
--  CORTEX_FUNCTIONS_USAGE_HISTORY). App code shipped in git v4.556.0.
--
--  Run as SNOW_ACCOUNTADMINS (the app's OWNING role). THREE steps, TOP-TO-BOTTOM:
--    STEP 1  apply the migration (re-derives SP_LOAD_MARTS_V27; guarded, needs V145).
--    STEP 2  backfill -- RUN BOTH STATEMENTS TOGETHER, in order (DELETE then CALL).
--    STEP 3  verify -- paste the STEP 3 grids back.
--
--  Prereq: V145 already applied (you applied it 2026-09-18). The STEP 1 guard RAISES
--  with -20146 if V145 is missing. Everything is idempotent / safe to re-run.
-- =====================================================================
...
-- =====================================================================
--  STEP 1 -- apply V146 (the migration, verbatim)
-- =====================================================================
...
-- =====================================================================
--  STEP 2 -- backfill (RUN BOTH STATEMENTS TOGETHER, in order)
--
--  The loader MERGE is UPSERT-only, so a plain re-CALL would INSERT the new
--  canonical per-model rows while leaving any stale frozen-view 'Functions' rows
--  (relabeled MODEL_NAME, or days the frozen view still returned) as ORPHANS that
--  DOUBLE-COUNT in every SUM-by-day/model reader. The scoped DELETE clears the
--  canonical-coverage window first; it is scoped to >= 2026-01-05 (the canonical
--  view's data horizon) so genuine pre-horizon 'Functions' history the canonical
--  view CANNOT reproduce is PRESERVED. Then reload the marts from the new arm.
-- =====================================================================
USE ROLE SNOW_ACCOUNTADMINS;
USE WAREHOUSE WH_ALFA_ADMIN;
...
-- =====================================================================
--  STEP 3 -- verify (paste all three grids back)
-- =====================================================================
USE ROLE SNOW_ACCOUNTADMINS;

-- (a) migration registered?
SELECT VERSION, LEFT(DESCRIPTION, 80) AS DESCRIPTION
FROM DBA_MAINT_DB.OVERWATCH.SCHEMA_VERSION WHERE VERSION = 146;

-- (b) reloaded Functions rows -- last 14 covered days, per model. Expect per-model
--     rows (e.g. llama3.1-8b), ONE row per (day, model), and NO stale 'n/a' row
--     sitting alongside a real model for the same day (that would be the orphan).
SELECT DAY, MODEL_NAME, REQUESTS, TOKENS, ROUND(CREDITS, 6) AS CREDITS
FROM DBA_MAINT_DB.OVERWATCH.FACT_AI_USAGE_DAILY
WHERE SOURCE = 'Functions' AND DAY >= DATEADD('day', -14, CURRENT_DATE())
ORDER BY DAY DESC, CREDITS DESC;

-- (c) cross-view credit sanity: new canonical CREDITS vs the interim AISQL
--     TOKEN_CREDITS over the same 30d window should be in the same ballpark
--     (both are Snowflake AI-function credit views). A big gap => tell me first.
SELECT 'AI_FUNCTIONS (new)' AS SRC, ROUND(SUM(CREDITS), 4) AS CREDITS
...
combined AS (
    SELECT USER_ID, USAGE_TIME, TOKEN_CREDITS, TOKENS, 'Snowsight' AS SOURCE
    FROM SNOWFLAKE.ACCOUNT_USAGE.CORTEX_CODE_SNOWSIGHT_USAGE_HISTORY
    WHERE USAGE_TIME >= (SELECT WINDOW_START FROM params)
      AND USAGE_TIME <  (SELECT WINDOW_END   FROM params)
    UNION ALL
