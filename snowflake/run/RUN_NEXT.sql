...
--
--  APPLY THE ONE PENDING MIGRATION: V125 (MFA-gap posture on the COALESCE
--  active-user canonical).
--
--  Context: V124 was applied on 2026-09-02, so MAX(VERSION) should read 124
--  before you start. V125 is a PROC-ONLY re-derivation of the byte-locked
--  SP_LOAD_MARTS_V27 (re-derived from V113), changing EXACTLY ONE line: the
--  security-posture MFA_GAP_USERS arm now filters active users with
--  COALESCE(U.DISABLED, FALSE) = FALSE instead of a bare U.DISABLED = FALSE,
--  matching every live reader on the Security page. This stops the warm-mart
--  MFA-gap KPI from reading one short when a user's ACCOUNT_USAGE.USERS.DISABLED
--  is NULL (never toggled) and they password-log-in without MFA.
--
--  It changes NO schema, provisions/alters NO compute, and is idempotent (safe
--  to re-run). After the CREATE OR REPLACE, this file re-runs the DAILY marts
--  once (SP_LOAD_MARTS_V27('DAILY', 3), matching the nightly task) so the
--  posture row re-stamps immediately instead of waiting for tonight's run.
--
--  HOW TO RUN: open this file on GitHub -> Copy raw -> paste into a Snowsight
--  worksheet -> Run All. Confirm SCHEMA_VERSION_AFTER = 125 and that the
--  MFA_GAP reconciliation RESULT shows RECONCILED = 'OK: mart matches live'.
--  Paste the three RESULT blocks back into the chat.
-- =====================================================================
...
-- (byte-identical to snowflake/migrations/ on main; only the MFA_GAP_USERS
--  WHERE-clause differs from V113 -- one line)
-- =====================================================================
...
-- =====================================================================
-- POST-APPLY RE-STAMP: re-run the DAILY marts so the security-posture row
-- (incl. MFA_GAP_USERS) re-stamps TODAY under the corrected proc. This is the
-- same call the nightly TASK makes (SCOPE='DAILY', DAYS_BACK=3). Idempotent.
-- =====================================================================
...
-- ---- POST-CHECK: expect SCHEMA_VERSION_AFTER = 125 ------------------
-- RESULT:
