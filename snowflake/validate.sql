...
WITH checks AS (
    SELECT 'V001..V123 applied' AS CHECK_NAME,
           IFF((SELECT COUNT(DISTINCT VERSION) FROM DBA_MAINT_DB.OVERWATCH.SCHEMA_VERSION WHERE VERSION BETWEEN 1 AND 123) = 123,
...
BEGIN
    -- (a) migration floor: the load-bearing minimum is V001..V088. This is a
    --     DELIBERATE floor (kept at 88 so a pre-existing install still validates),
    --     NOT the repo tip — the header SELECT above reports the full V001..V<tip>
    --     count for humans; only this 88 check RAISEs and gates deploy.
