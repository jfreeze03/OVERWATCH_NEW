...
WITH checks AS (
    SELECT 'V001..V117 applied' AS CHECK_NAME,
           IFF((SELECT COUNT(DISTINCT VERSION) FROM DBA_MAINT_DB.OVERWATCH.SCHEMA_VERSION WHERE VERSION BETWEEN 1 AND 117) = 117,
...
DECLARE
    ...
    e_migrations  EXCEPTION (-20011, 'VALIDATE FAIL: V001..V117 migration floor not met — run missing migrations');
...
BEGIN
    -- (a) migration floor: V001..V117 all applied.
