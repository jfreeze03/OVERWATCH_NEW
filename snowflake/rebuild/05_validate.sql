...
WITH checks AS (
    SELECT 'V001..V071 applied' AS CHECK_NAME,
           IFF((SELECT COUNT(DISTINCT VERSION) FROM DBA_MAINT_DB.OVERWATCH.SCHEMA_VERSION WHERE VERSION BETWEEN 1 AND 71) = 71,
    ...
    UNION ALL
    -- 29 = full seeded rule set across V001..V071 (V004 seeds 7; V007/V009/
...
DECLARE
    ...
    e_migrations  EXCEPTION (-20011, 'VALIDATE FAIL: V001..V071 migration floor not met — run missing migrations');
...
BEGIN
    -- (a) migration floor: V001..V071 all applied.
    SELECT COUNT(DISTINCT VERSION) INTO :n_versions
      FROM DBA_MAINT_DB.OVERWATCH.SCHEMA_VERSION
     WHERE VERSION BETWEEN 1 AND 71;
    IF (n_versions < 71) THEN RAISE e_migrations; END IF;
