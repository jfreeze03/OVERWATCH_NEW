...
WITH checks AS (
    SELECT 'V001..V077 applied' AS CHECK_NAME,
           IFF((SELECT COUNT(DISTINCT VERSION) FROM DBA_MAINT_DB.OVERWATCH.SCHEMA_VERSION WHERE VERSION BETWEEN 1 AND 77) = 77,
...
DECLARE
    ...
    e_migrations  EXCEPTION (-20011, 'VALIDATE FAIL: V001..V077 migration floor not met — run missing migrations');
...
BEGIN
    -- (a) migration floor: V001..V077 all applied.
    SELECT COUNT(DISTINCT VERSION) INTO :n_versions
      FROM DBA_MAINT_DB.OVERWATCH.SCHEMA_VERSION
     WHERE VERSION BETWEEN 1 AND 77;
    IF (n_versions < 77) THEN RAISE e_migrations; END IF;
