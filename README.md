...
snowflake/migrations/V075__security_operating_model.sql -- Security operating model: materializes bounded login/change evidence; snapshots Trust Center deltas; adds identity, client, and egress policy registries; and adds inherited-access review campaigns with append-only decision history. Owner-applied; never run by the app.
