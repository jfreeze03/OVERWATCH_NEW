...
account-clock window — is migration-gated and tracked separately):
  ...
  ROI-1  Brief verified-savings tile (now "Verified savings run-rate", Next-Fifty #3) does not paint a
         false green when app cost is a degenerate 0
"""
...
def test_brief_roi_zero_denominator_is_unmeasured():
    ...
    blk = b.split('"Verified savings run-rate"', 1)[0][-700:]
