...
def test_cortex_code_reads_are_probes_with_a_truthful_note():
    ...
    # P2/P9 (2026-07-31): the Cortex Code live leg is ONE superset 365d fetch (the
    # scans are window-flat, so one read is strictly cheaper than two) sliced in
    # pandas — one probe read where there used to be two. (The user-attribution
    # panel is live-first as of 2026-09-13; run_mart_first still backs the sibling
    # AI-Functions / share panels.) The probe + the truthful note are what this test
    # guards, not the number of scans.
    assert "probe=True" in ai                                      # the live leg still probes
    assert "run_mart_first" in ai                                  # used by the sibling AI-Functions / share panels
