...
def test_release_compare_humanizes_durations_and_keeps_units():
    df = pd.DataFrame([
        {"PERIOD": "BEFORE", "QUERY_COUNT": 1000, "FAILED_COUNT": 10,
         "P95_ELAPSED_SEC": 3661.0, "QUEUED_SEC": 90.0, "SPILL_REMOTE_GB": 12.5},
        {"PERIOD": "AFTER", "QUERY_COUNT": 1200, "FAILED_COUNT": 30,
         "P95_ELAPSED_SEC": 45.0, "QUEUED_SEC": 90.0, "SPILL_REMOTE_GB": 12.5},
    ])
    ...
    # Non-duration metrics keep their own unit.
    assert rows["Remote spill (GB)"]["Before"] == "12.50 GB"
