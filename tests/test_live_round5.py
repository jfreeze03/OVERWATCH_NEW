...
def test_multiselect_chips_are_readable():
    ...
    # v4.155 re-theme: the chip tint follows the iris accent, still a dark chip
    # (never the pale wash) with explicitly readable light text.
    assert "rgba(143,138,255,0.18)" in theme           # dark chip, not the pale wash
    assert 'tag"] span { color:#e7e5ff' in theme       # readable text
