...
def test_admin_migration_contract_matches_the_repo():
    ...
    repo_versions = {
        ...
        for p in (_ROOT / "snowflake" / "migrations").glob("V[0-9]*.sql")
    }
...
_LIVE_SCAN_BUDGETS = {
    ...
    "app/ui/pages/cost_parts/optimize.py": 5,   # +1 v4.30: toggled clustering-spend scan (COST_DB recon R7; on-demand, labeled); +1 v4.269 (UX sweep #24): storage-growth movers row-click drills the clicked database's per-table storage (table_storage_breakdown; ACCOUNT_USAGE.TABLE_STORAGE_METRICS + TABLE_DML_HISTORY) — interaction-gated on the row click, not first paint, mirrors spend.py's existing _storage_table_drill; +1 v4.546 (cost-audit): per-table at-rest pruning candidates (TABLE_PRUNING_HISTORY) — inside the existing "Run query-efficiency scan" toggle, off first paint, probe-gated
    ...
    "app/ui/pages/cost_parts/ai_chargeback.py": 5,  # -1 v4.50: the storage-tier live fallback moved to spend.py with the storage panels; +1 v4.543 (cost-audit): the per-user AI-quota block read (QUOTA_ACCESS_BLOCK_HISTORY) — the one account-wide read for native per-user AI quotas, probe-gated AND behind the whole tab's "Load AI user attribution" toggle, so off first paint / not ambient
    ...
    "app/ui/pages/security.py": 31,  # +1 v4.352 (security hunt H4): the Auditor export pack's MFA sheet gains a live LOGIN_HISTORY proof (users_without_mfa_live) so a deployed-but-empty FACT_LOGIN_DAILY never writes a 0-row "all clear" CSV — gated on an empty mart AND only on the build-on-click export, never first paint; the source LABEL counts toward this proxy. +4 v4.41/r25 (owner picked #6+#7): new-network batch rider on Access + Egress lazy section (DATA_TRANSFER_HISTORY, UNLOAD scan) — zero first-paint cost, all click-gated. +1 v4.240.0 (owner ask): recent-grant-changes feed (GRANTS_TO_USERS + GRANTS_TO_ROLES) on the non-default Changes section, hourly-cached — not first paint. +2 perf-batch (S-batch rec #23-drill): NOT new scans — the role-holder revoke-safety drill's two ALREADY-EXISTING interaction-gated reads (role_holders/role_privileges) now prefetch via run_batch, so each ACCOUNT_USAGE.GRANTS_* source LABEL appears twice (batch spec + run() fallback); the reachable-table set is unchanged (test_v451_trust). +1 v4.299 (Upgrade Board P1 #18): exfiltration-score panel adds one QUERY_HISTORY per-event scan (unload_risk_events) TOGGLE-gated inside the already-lazy Egress section — off first paint, QUERY_HISTORY already reachable per test_v451_trust so the reachable table set is unchanged; sits beside the existing UNLOAD day-grain scan. +4 v4.300 (Upgrade Board P1 #20): object-tag governance panel adds TABLES+TAG_REFERENCES reads (object_tag_coverage/untagged_objects) + a TAG_REFERENCES probe — PROBE-gated and in the Decision-queue governance block below the score/trend panels, NOT first paint; the +4 count is the 3 source labels plus one setup-caption string mentioning the view (a display string, not extra scans); reachable-table pin updated deliberately in test_v451_trust
}
