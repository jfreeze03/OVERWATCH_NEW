...
<style>
:root {
  /* v4.155 recolor: deeper cool neutrals + indigo->cyan brand accent (was the
     stock sky-blue). Chrome + accent hex live in app/ui/palette.py and are
     mirrored here + in .streamlit/config.toml (tests/test_palette_drift.py). */
  --ow-bg:#090d17; --ow-surface:#0f1626; --ow-raised:#17203a;
  ...
  /* rec 15 (a11y): --ow-ink-mute lifted #6b7a90 -> #8593a8 so the small (0.62-0.70rem)
     muted labels clear WCAG AA 4.5:1 on every surface they land on (bg 6.2, surface 5.8,
     raised 5.2). Every muted label references this one token, so one change fixes all. */
  --ow-ink:#e8eef7; --ow-ink-soft:#aab6c8; --ow-ink-mute:#8593a8;
  --ow-accent:#6c8cff; --ow-accent2:#22d3ee;
  --ow-ok:#34d399; --ow-warn:#fbbf24; --ow-bad:#fb7185; --ow-info:#6c8cff;
  --ow-ok-dim:rgba(52,211,153,0.14); --ow-warn-dim:rgba(251,191,36,0.14);
  --ow-bad-dim:rgba(251,113,133,0.14); --ow-info-dim:rgba(108,140,255,0.14);
...
.ow-src-badge--live { color:#6c8cff; border-color:rgba(108,140,255,0.3); }
...
.ow-help:focus-visible { box-shadow:0 0 0 2px rgba(108,140,255,0.45); }
...
div[data-testid="stVerticalBlockBorderWrapper"]:has(.ow-scope-active){
  ...
  box-shadow:0 0 0 1px rgba(108,140,255,0.22),var(--ow-shadow);}
...
.ow-brand-dot { width:11px; height:11px; border-radius:999px;
  ...
  box-shadow:0 0 10px rgba(108,140,255,0.9),0 0 2px rgba(108,140,255,1); animation:ow-pulse 2.8s ease-in-out infinite; }
...
.stButton > button:hover { border-color:var(--ow-accent); box-shadow:0 6px 18px -10px rgba(108,140,255,0.6); }
...
.stMultiSelect [data-baseweb="tag"] { background:rgba(108,140,255,0.16) !important;
  border:1px solid rgba(108,140,255,0.55) !important; border-radius:var(--ow-r-sm); }
...
section[data-testid="stSidebar"] div[role="radiogroup"] label:has(input:checked) {
  background:linear-gradient(90deg,rgba(108,140,255,0.18),transparent); box-shadow:inset 3px 0 0 var(--ow-accent); }
