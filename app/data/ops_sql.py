...
SELECT
    ...
    ROUND(SUM(COMPILATION_TIME) / 1000.0, 1) AS TOTAL_COMPILE_SEC,
    ...
    -- warehouse execution (a SYSTEM$/SHOW/INFORMATION_SCHEMA/driver call). Its own signal because
    -- COMPILE_RUN_PCT gates on >=1s compile, but chatter is sub-second all-compile (FBE ~540ms).
    AVG(IFF(COALESCE(COMPILATION_TIME, 0) / NULLIF(TOTAL_ELAPSED_TIME, 0) >= 0.7
            AND COALESCE(EXECUTION_TIME, 0) <= 500, 1, 0)) AS COMPILE_DOMINANT_RUN_PCT,
...
-- order by the GREATER of the compute (exec) and compile footprints, so a compile-dominated
-- family (metadata chatter) with little execution is still a top-500 candidate, not starved out.
ORDER BY GREATEST(SUM(EXECUTION_TIME), SUM(COMPILATION_TIME)) DESC
LIMIT 500
"""
