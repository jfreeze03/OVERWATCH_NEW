...
PLAYBOOKS: dict[str, str] = {
    ...
    "OPS_SCAN_DEGRADED": (
        ...
        "confirm no new `rule_block_failed` row, then resolve. Since V157 a hand CALL of the hourly scan "
        "obeys its cadence gates: SEC_CRED_EXPIRY / SEC_NEW_EXPOSURE run only in the 01, 05, 09, 13, 17 and "
    ...
    "OPS_PIPELINE_DEGRADED": (
        ...
        "arrive up to ~3h after it crosses its limit, and a stale or idle episode that ends between two checks "
        "is not raised (a logged loader failure is still caught within 24h).\n\n"
    ...
    "SEC_NEW_EXPOSURE": (
        ...
        "(which itself can lag about 2h). A grant revoked before the next check (an exposure shorter than "
        "~4h) is never raised — Security > Changes and GRANTS_TO_ROLES history still show it.\n\n"
    ...
    "PIPE_ETL_CYCLE_LATE": (
        ...
        "this cycle, so a later re-run of it does not hold the night open). The cache is as of the scan's last "
        "in-window run: after HOUR(ETL_SLA_TARGET_HHMM + 3h) it next refreshes at the 15:00 pass, then at the "
        "window start, so past that hour trust *Tonight at a glance* (live) over this query.\n"
