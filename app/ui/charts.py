...
# Starts at a visible dark-orange (NOT the page background #0b0e14) so a low-but-
...
.dag-host {
  ...
  background: #0b0e14; border: 1px solid rgba(148,163,184,.28); border-radius: 6px;
}
...
.dag-toolbar {
  ...
  padding: 5px; background: rgba(20,24,33,.94); border: 1px solid rgba(148,163,184,.28);
...
.dag-toolbar input {
  ...
  border-radius: 5px; background: #0b0e14; color: #e9edf5; font: 500 12px Inter, sans-serif;
}
.dag-toolbar input:focus-visible { outline: 2px solid #6e8bff; outline-offset: 1px; }
.dag-toolbar button {
  min-width: 34px; height: 32px; padding: 0 9px; border: 1px solid rgba(148,163,184,.32);
  border-radius: 5px; background: #1c2130; color: #e9edf5;
  font: 650 13px/1 Inter, "Segoe UI", sans-serif; cursor: pointer;
}
.dag-toolbar button:hover { border-color: #6e8bff; background: #242c3e; }
.dag-toolbar button:focus-visible { outline: 2px solid #6e8bff; outline-offset: 2px; }
svg {
...
.edge {
  fill: none; stroke: #8a97ab; stroke-width: 1.5; opacity: .72;
  marker-end: url(#arrow);
}
.node rect {
  fill: #1c2130; stroke: var(--node-color); stroke-width: 2;
  filter: url(#shadow);
}
...
.node-title { fill: #e9edf5; font-size: 13px; font-weight: 700; }
.node-meta { fill: #a9b4c4; font-size: 10px; }
  ...
  <svg id="dag" tabindex="0" role="img"
    ...
    <defs>
      <marker id="arrow" markerWidth="8" markerHeight="8" refX="7" refY="3"
        ...
        <path d="M0,0 L0,6 L8,3 z" fill="#8a97ab"/>
      </marker>
...
def event_timeline(df: pd.DataFrame) -> None:
    ...
    dots = alt.Chart().mark_point(size=110, opacity=0.95, filled=True,
                stroke="#0b0e14", strokeWidth=0.6).encode(color=color, shape=shape, **common)
...
def operational_replay(df: pd.DataFrame) -> None:
    ...
    focus = (
        alt.Chart(data)
        .mark_point(size=125, filled=True, stroke="#0b0e14", strokeWidth=0.7)
        .encode(
...
def workload_portfolio(df: pd.DataFrame) -> None:
    ...
    chart = (
        alt.Chart(data)
        .mark_circle(opacity=0.82, stroke="#0b0e14", strokeWidth=0.8)
        .encode(
            x=alt.X(
