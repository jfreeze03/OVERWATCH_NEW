...
def _overwatch_theme() -> dict:
    return {
        "config": {
            ...
            "range": {
                # rec50 (#e879f9/#22d3ee/#a3e635 chart-only). v4.155: the iris accent
                # replaced cyan, so the old #c084fc purple moved to fuchsia and the
                # near-accent ACCENT2 slot became the retired cyan — eight hues stay
                # tellable-apart in one categorical scale.
                "category": [_ACCENT, palette.OK, "#e879f9", palette.WARN, palette.BAD,
                              "#22d3ee", "#a3e635", palette.HIGH],
...
.dag-host {
  ...
  background: #131215; border: 1px solid rgba(171,168,182,.28); border-radius: 6px;
}
...
.dag-toolbar {
  ...
  padding: 5px; background: rgba(26,25,29,.94); border: 1px solid rgba(171,168,182,.28);
  border-radius: 6px; box-shadow: 0 6px 20px rgba(0,0,0,.35);
}
.dag-toolbar input {
  width: 190px; height: 32px; padding: 0 9px; border: 1px solid rgba(171,168,182,.32);
  border-radius: 5px; background: #131215; color: #eeedf2; font: 500 12px Inter, sans-serif;
}
.dag-toolbar input:focus-visible { outline: 2px solid #8f8aff; outline-offset: 1px; }
.dag-toolbar button {
  min-width: 34px; height: 32px; padding: 0 9px; border: 1px solid rgba(171,168,182,.32);
  border-radius: 5px; background: #232228; color: #eeedf2;
  font: 650 13px/1 Inter, "Segoe UI", sans-serif; cursor: pointer;
}
.dag-toolbar button:hover { border-color: #8f8aff; background: #2a2930; }
.dag-toolbar button:focus-visible { outline: 2px solid #8f8aff; outline-offset: 2px; }
svg {
...
.edge {
  fill: none; stroke: #948fa3; stroke-width: 1.5; opacity: .72;
  marker-end: url(#arrow);
}
.node rect {
  fill: #232228; stroke: var(--node-color); stroke-width: 2;
  filter: url(#shadow);
}
...
.node-title { fill: #eeedf2; font-size: 13px; font-weight: 700; }
.node-meta { fill: #b6b3c0; font-size: 10px; }
  ...
  <svg id="dag" tabindex="0" role="img"
    ...
    <defs>
      <marker id="arrow" markerWidth="8" markerHeight="8" refX="7" refY="3"
        ...
        <path d="M0,0 L0,6 L8,3 z" fill="#948fa3"/>
      </marker>
    ...
    *,
    ...
    bars = (alt.Chart().mark_bar(color=grad, cornerRadiusEnd=2, size=bar_size)
            .encode(x=enc_x,
                    ...
                    tooltip=tip))
    avg = (alt.Chart().mark_line(color=_TITLE, strokeWidth=2, interpolate="monotone")
