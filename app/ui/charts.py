...
def _overwatch_theme() -> dict:
    return {
        "config": {
            ...
            "range": {
                "category": [_ACCENT, palette.OK, "#22d3ee", palette.WARN, palette.BAD,
                              palette.ACCENT2, "#a3e635", palette.HIGH],  # rec50 (#22d3ee/#a3e635 chart-only)
...
.dag-host {
  ...
  background: #0e1016; border: 1px solid rgba(148,163,184,.28); border-radius: 6px;
}
...
.dag-toolbar {
  ...
  padding: 5px; background: rgba(22,25,34,.94); border: 1px solid rgba(148,163,184,.28);
...
.dag-toolbar input {
  ...
  border-radius: 5px; background: #0e1016; color: #e8eef7; font: 500 12px Inter, sans-serif;
}
.dag-toolbar input:focus-visible { outline: 2px solid #818cf8; outline-offset: 1px; }
.dag-toolbar button {
  min-width: 34px; height: 32px; padding: 0 9px; border: 1px solid rgba(148,163,184,.32);
  border-radius: 5px; background: #1e222e; color: #e8eef7;
  font: 650 13px/1 Inter, "Segoe UI", sans-serif; cursor: pointer;
}
.dag-toolbar button:hover { border-color: #818cf8; background: #262b3a; }
.dag-toolbar button:focus-visible { outline: 2px solid #818cf8; outline-offset: 2px; }
svg {
...
.node rect {
  fill: #1e222e; stroke: var(--node-color); stroke-width: 2;
  filter: url(#shadow);
}
