...
# Starts at a visible dark-orange (NOT the page background palette.BG) so a low-but-
...
def _overwatch_theme() -> dict:
    return {
        "config": {
            ...
            "range": {
                # v4.155: with the indigo accent, ACCENT2 (indigo-300) became a
                # near-duplicate of series 1, so slot 6 is chart-only violet and
                # slot 3 is INFO sky — 8 distinguishable hues, palette-sourced.
                "category": [_ACCENT, palette.OK, palette.INFO, palette.WARN, palette.BAD,
                              "#c084fc", "#a3e635", palette.HIGH],  # rec50 (#c084fc/#a3e635 chart-only)
...
def _task_dag_markup(df: pd.DataFrame, shape: TaskGraphShape, *, height: int) -> str:
    ...
    return (
        ...
        # Chrome sourced from the one palette (rec50) so the DAG iframe can
        # never drift from the app shell again (it carried stale literals).
        .replace("__BG__", palette.BG)
        .replace("__SURFACE_GLASS__", _rgba(palette.SURFACE, 0.94))
        .replace("__RAISED_HOVER__", "#212530")  # RAISED lifted one step for hover
        .replace("__RAISED__", palette.RAISED)
        .replace("__INK_SOFT__", palette.INK_SOFT)
        .replace("__INK_MUTE__", palette.INK_MUTE)
...
.dag-host {
  ...
  background: __BG__; border: 1px solid rgba(148,163,184,.28); border-radius: 6px;
}
...
.dag-toolbar {
  ...
  padding: 5px; background: __SURFACE_GLASS__; border: 1px solid rgba(148,163,184,.28);
...
.dag-toolbar input {
  ...
  border-radius: 5px; background: __BG__; color: __INK__; font: 500 12px Inter, sans-serif;
}
.dag-toolbar input:focus-visible { outline: 2px solid __ACCENT__; outline-offset: 1px; }
.dag-toolbar button {
  min-width: 34px; height: 32px; padding: 0 9px; border: 1px solid rgba(148,163,184,.32);
  border-radius: 5px; background: __RAISED__; color: __INK__;
  font: 650 13px/1 Inter, "Segoe UI", sans-serif; cursor: pointer;
}
.dag-toolbar button:hover { border-color: __ACCENT__; background: __RAISED_HOVER__; }
.dag-toolbar button:focus-visible { outline: 2px solid __ACCENT__; outline-offset: 2px; }
svg {
...
.edge {
  fill: none; stroke: __INK_MUTE__; stroke-width: 1.5; opacity: .72;
  marker-end: url(#arrow);
}
.node rect {
  fill: __RAISED__; stroke: var(--node-color); stroke-width: 2;
  filter: url(#shadow);
}
...
.node-title { fill: __INK__; font-size: 13px; font-weight: 700; }
.node-meta { fill: __INK_SOFT__; font-size: 10px; }
  ...
  <svg id="dag" tabindex="0" role="img"
    ...
    <defs>
      <marker id="arrow" markerWidth="8" markerHeight="8" refX="7" refY="3"
        ...
        <path d="M0,0 L0,6 L8,3 z" fill="__INK_MUTE__"/>
      </marker>
