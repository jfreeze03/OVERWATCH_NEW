...
# the drift test). BAD doubles as CRITICAL. INFO is the informational sky blue —
# v4.155 decoupled it from the accent so "live/informational" reads as sky while
# the primary accent moved to indigo (see below).
OK = "#34d399"        # healthy / better / success            (--ow-ok)
WARN = "#fbbf24"      # watch / MEDIUM / amber                 (--ow-warn)
BAD = "#fb7185"       # act-now / CRITICAL / worse / red       (--ow-bad)
INFO = "#4bb4f2"      # informational / sky                    (--ow-info)

# Accents (mirror --ow-accent / --ow-accent2). v4.155 palette refresh: the
# primary accent moved from sky-cyan (#38bdf8/#22d3ee) to an indigo->violet pair
# for a calmer, more distinctive "ops console" identity. Severity hues (OK/WARN/
# BAD) are deliberately unchanged — they carry meaning, not brand.
ACCENT = "#6e8bff"    # indigo — primary accent / links / focus
ACCENT2 = "#a78bfa"   # violet — gradient partner (buttons, section pills, bars)
...
# v4.155 refresh: the base shifted from a blue-heavy navy to a neutral graphite
# so cards read as slate panels, not tinted blue, and the indigo accent stands
# out. Contrast steps (bg < surface < raised) are preserved; INK_MUTE stays
# WCAG AA (>=4.5:1) on the darkest small-text surface (raised).
BG = "#0b0e14"        # app background       (--ow-bg)
SURFACE = "#141821"   # cards / secondary bg (--ow-surface)
RAISED = "#1c2130"    # raised surfaces      (--ow-raised)
INK = "#e9edf5"       # primary text         (--ow-ink)
INK_SOFT = "#a9b4c4"  # secondary text       (--ow-ink-soft)
INK_MUTE = "#8a97ab"  # muted labels         (--ow-ink-mute)
