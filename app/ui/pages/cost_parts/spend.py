...
def _spend_tab(company: str, days: int, rate: float, ai_rate: float,
    ...
    with st.expander("Why totals differ across pages (and vs Snowsight)"):
        ...
        st.markdown(
            ...
            "cloud-services rebate applied. Warehouse metering is the only grain Snowflake scopes "
            "per warehouse, so Overview's company-scoped KPI is a slice of THIS figure priced on "
            "unadjusted usage — it reads at or below it, rebate-free, never equal. The remaining "
            f"{format_usd(other_usd)} (serverless, AI, replication, reader) has no warehouse to "
            "scope by.\n"
