...
- **Section A (live):** tasks, alerts, procs, functions, views, transient
  facts/marts. Safe anytime — re-run the migrations in order (V001..V130) and the loaders repopulate.
...
- **Schema dropped:** `UNDROP SCHEMA DBA_MAINT_DB.OVERWATCH;` first. If gone,
  re-run all migrations in order (V001..V130) + roles.sql + validate.sql; facts refill from
